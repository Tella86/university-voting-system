...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $photo = $_FILES['photo'];
    $position = $_POST['position']; // Position selected by the candidate

    // Validate file upload
    $upload_dir = '../assets/images/';
    $file_path = $upload_dir . basename($photo['name']);
    if (move_uploaded_file($photo['tmp_name'], $file_path)) {
        $stmt = $pdo->prepare("INSERT INTO candidates (user_id, election_id, name, manifesto, photo, position) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$_SESSION['user_id'], $election_id, $name, $manifesto, $file_path, $position]);
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <select name="position" class="form-select" required>
                    ...
                    <option value="college_babaa">College Baba</option>
                    <option value="college_mamaa">College Mama</option>
                    <option value="class_secretary">Class Secretary</option>
                    <option value="dorm_mother">Dorm Mother</option>
                </select>
            </div>
